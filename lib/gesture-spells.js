...
export class GestureSpells {
    ...
    loadModel(load) {
        if (load) {
            ...
            this.loaded = tf.loadModel('../assets/model/spelling.json');
        }
    }
    ...
    normCurve(Curve) {
        ...
        const factor = this.resolution / GestureSpells.distance([maxX, maxY], [minX, minY]);
    ...
    normSpell(theSpell) {
        ...
        const factor = this.resolution / GestureSpells.distance([maxX, maxY], [minX, minY]);
        return this.normCurve(GestureSpells.toCurve(theSpell.map(va => [
            Math.floor((va[0] - minX) * factor),
            Math.floor((va[1] - minY) * factor),
        ]).reduce((a, val, i, arr) => {
            if (i === 0
                || i + 1 === arr.length
                || !GestureSpells.isInALine(a.slice(-1), val, arr[i + 1])) {
                a.push(val);
            }
            return a;
        }, [])));
    ...
    static isInALine(a, b, c) {
        if (a && b && c) {
            return (GestureSpells.distance(a, b) + GestureSpells.distance(b, c))
            === GestureSpells.distance(a, c);
        }
        return false;
    }
    recognise(arrayOfxy) {
